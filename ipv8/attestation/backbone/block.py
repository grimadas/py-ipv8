...
SKIP_ATTRIBUTES = {'key', 'serializer', 'crypto', '_transaction', '_logger', '_previous', '_links'}
...
class NoodleBlock(object):
    ...
    def __init__(self, data=None, serializer=default_serializer):
        ...
        if data is None:
            ...
            self.signature, self.timestamp, self.insert_time = data[8], data[9], data[10]
    ...
    def from_payload(cls, payload, serializer):
        ...
        return cls([payload.type, payload.transaction, payload.public_key, payload.sequence_number,
                    payload.previous, payload.links,
                    payload.com_id, payload.com_seq_num,
    ...
    @property
    def block_id(self):
        return b"%s.%d:%s" % (hexlify(self.public_key), self.sequence_number, hexlify(self.hash))

    @property
    def is_peer_genesis(self):
        return self.sequence_number == GENESIS_SEQ and (0, GENESIS_HASH) in self.previous
    ...
    def pack(self, signature=True):
        ...
        """
        args = [self.type, self._transaction, self.public_key, self.sequence_number,
                self._previous, self._links, self.com_id, self.com_seq_num,
                self.signature if signature else EMPTY_SIG, self.timestamp]
    ...
    def pack_db_insert(self):
        ...
        return (self.type, database_blob(self._transaction), database_blob(self.public_key),
                self.sequence_number, database_blob(self._previous), database_blob(self._links),
                database_blob(self.com_id), self.com_seq_num, database_blob(self.signature), self.timestamp,
